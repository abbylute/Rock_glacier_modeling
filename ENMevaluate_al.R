# This code was adapted from ENMeval::ENMevaluate to work with SWD instead of rasters

ENMevaluate_al <- function (occ, env = NULL, bg.coords = NULL, occ.grp = NULL, bg.grp = NULL, 
          ...
          parallel = FALSE, numCores = NULL, progbar = TRUE, updateProgress = FALSE, outdir=NULL,
          ...) 
{
  ptm <- proc.time()
  if (is.null(method)) {
  ...
  output <- tuning_al(occ, env, bg.coords, occ.grp, bg.grp, method, 
                    algorithm, args, args.lab, categoricals, aggregation.factor, 
                    kfolds, bin.output, clamp, alg, rasterPreds, parallel, 
                    numCores, progbar, updateProgress, userArgs, outdir)
