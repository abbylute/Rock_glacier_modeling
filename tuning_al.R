# This code was adapted from ENMeval::tuning to work with SWD instead of raster format

tuning_al <- function (occ, env, bg.coords, occ.grp, bg.grp, method, algorithm, 
          args, args.lab, categoricals, aggregation.factor, kfolds, 
          bin.output, clamp, alg, rasterPreds, parallel, numCores, 
          progbar, updateProgress, userArgs, outdir) 
{
  ...
  if (parallel == TRUE) {
    ...
    if (algorithm == "maxnet") {
      ...
      out <- foreach(i = seq_len(length(args)), .packages = c("dismo", 
                                                              "raster", "ENMeval", "rJava")) %dopar% {
                                                                modelTune.maxentJar_al(pres, bg, env, nk, group.data, 
                                                                                    args[[i]], userArgs, rasterPreds, clamp, categoricals, outdir)
                                                              }
    }
    stopCluster(c1)
  }
    ...
    for (i in 1:length(args)) {
      ...
      if (algorithm == "maxnet") {
        ...
        out[[i]] <- modelTune.maxentJar_al(pres, bg, env, 
  ...
  if (rasterPreds) {
    ...
    predictive.maps <- (sapply(out, function(x) x[[3]]))
  }
  else {
  ...
  aicc <- calc.aicc_al(nparam, occ, predictive.maps)
  features <- args.lab[[1]]
  rm <- args.lab[[2]]
  ...
  if (rasterPreds == TRUE) {
    predictive.maps <- as.data.frame(predictive.maps)
    names(predictive.maps) <- settings 
  }
  results <- ENMevaluation(algorithm = alg, results = res, 
                           predictions = stack(raster(matrix(0))), models = full.mods, partition.method = method, 
                           occ.pts = occ, occ.grp = group.data[[1]], bg.pts = bg.coords, 
                           bg.grp = group.data[[2]])
  output <- list()
  output$results <- results
  output$predictions <- predictive.maps
  return(output)
}
